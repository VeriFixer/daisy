...
llm_cost_stub.get_cost_statistics(llm_configurations.MODEL_REGISTRY["claude-opus-4.5"])
...
def perform_prompt_localization(llm : llm_configurations.LLM, run_options : llm_pipeline.RunOptions, global_options : llm_pipeline.GlobalOptions):
...
def perform_prompt_inferece(llm : llm_configurations.LLM, run_options : llm_pipeline.RunOptions, global_options : llm_pipeline.GlobalOptions):
...
print("\n Running LAUREL WITH HAIKU\n")
run_options.localization = llm_pipeline.LocStrategies.LAUREL
perform_prompt_localization(llmhaiku, run_options, global_options)
perform_prompt_inferece(llmhaiku, run_options, global_options)
perform_verification(llmhaiku, run_options, global_options)

print("\n Running LAUREL BETTER WITH HAIKU\n")
run_options.localization = llm_pipeline.LocStrategies.LAUREL_BETTER
perform_prompt_localization(llmhaiku, run_options, global_options)
perform_prompt_inferece(llmhaiku, run_options, global_options)
perform_verification(llmhaiku, run_options, global_options)

print("\n Running ORACLE WITH HAIKU\n")
run_options.localization = llm_pipeline.LocStrategies.ORACLE
perform_prompt_localization(llmhaiku, run_options, global_options)
perform_prompt_inferece(llmhaiku, run_options, global_options)
perform_verification(llmhaiku, run_options, global_options)

print("\n Running LLM WITH HAIKU\n")
run_options.localization = llm_pipeline.LocStrategies.LLM
perform_prompt_localization(llmhaiku, run_options, global_options)
perform_prompt_inferece(llmhaiku, run_options, global_options)
perform_verification(llmhaiku, run_options, global_options)

print("\n Running LLM EXAMPLE WITH HAIKU\n")
...
print("\n Running LAUREL BETTER WITH OPUS\n")
run_options.localization = llm_pipeline.LocStrategies.LAUREL_BETTER
perform_prompt_localization(llmopus, run_options, global_options)
perform_prompt_inferece(llmopus, run_options, global_options)
perform_verification(llmopus, run_options, global_options)

print("\n Running ORACLE WITH OPUS\n")
run_options.localization = llm_pipeline.LocStrategies.ORACLE
perform_prompt_localization(llmopus, run_options, global_options)
perform_prompt_inferece(llmopus, run_options, global_options)
perform_verification(llmopus, run_options, global_options)

print("\n Running LLM EXAMPLE WITH OPUS\n")
run_options.examples_to_augment_prompt_type_pos = llm_pipeline.ExampleStrategies.DYNAMIC
run_options.examples_weight_of_error_message_pos = 0.25
run_options.number_examples_to_add_pos = 3
run_options.localization = llm_pipeline.LocStrategies.LLM_EXAMPLE
...
print("\n Running HYBRID WITH OPUS\n")
# Hybrid method uses LAUREL BETTER and the other in conjugation adding one solution to the other
run_options.localization = llm_pipeline.LocStrategies.HYBRID
run_options.examples_to_augment_prompt_type_pos=llm_pipeline.ExampleStrategies.DYNAMIC # "RANDOM", "DYNAMIC", "NONE", "EMBEDDED" "TFIDF" all options     
run_options.examples_weight_of_error_message_pos = 0.25
run_options.number_examples_to_add_pos=3
