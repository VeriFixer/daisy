...
def validate_folder_path(file_path, error_message):
...
def prepare_output_file(program_name, destination_path):
    program_converted_name = program_name.replace(".", "_").replace(" ", "_")
...
def process_assertions(dafny_exec, program_info_with_assertions, program_dst_folder, program_path, temp_dir, max_assertions_to_remove):
    dataset_list = []
    for method_info in program_info_with_assertions["Method"] + program_info_with_assertions["Function"]:
        assertion_infos = method_info["assertion"]

        if(assertion_infos!= [] and len(assertion_infos) > 0):
          dataset_list = dataset_list + get_assertion_to_test.process_assertions_method(dafny_exec, program_dst_folder,  program_path, method_info, assertion_infos, temp_dir, max_assertions_to_remove)
    
    return dataset_list

# number_of_assertions_to_remove if -1 remove all
def dafny_file_dataset_generator(dafny_exec, dafny_program_assertion_folder, dafny_destination_dataset_path, temp_dir, max_assertions_to_remove):
    ...
    file_assertions_dict = dafny_read_assertions_xml.extract_assertion(assertions_text)

    if file_assertions_dict is None:
        return 0
    ...
    process_assertions(dafny_exec, file_assertions_dict, dafny_program_dst, dafny_program, temp_dir, max_assertions_to_remove)




import concurrent.futures
def run_dataset_generation(max_to_assert_to_remove):
    ...
    def process_dir(dafny_dir, dafny_exec, assertion_dataset, temp_folder, max_remove):
